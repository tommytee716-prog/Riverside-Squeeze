...
function useOrders() {
  ...
  // Listen for live orders from Firebase
  useEffect(() => {
  ...
  const placeOrder = async (items, glitter, customerName, phone) => {
    ...
    const ordersRef = ref(db, "orders");
    const newRef = await push(ordersRef, order);
  ...
  const completeOrder = async (id, firebaseKey) => {
    ...
    // Move to completed
    const completedRef = ref(db, "completed");
    await push(completedRef, { ...order, status: "ready" });
    // Remove from orders
    ...
    setSales(s => [...s, {
      ...
      weather: "Live", isSample: false, orderId: order.id
    }]);
  };
