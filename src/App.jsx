...
function MusicPlayer({ songOfDay }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      >
        {playing ? "🎵 Now Playing..." : "🎵 Play Today's Song"}
      </a>
    </div>
  );
}
...
function useOrders() {
  ...
  const placeOrder = (items, glitter, customerName, phone) => {
    ...
    const order = { id: Date.now(), number: num, items, glitter, total, discount, freeItem, status: "pending", time: new Date(), location: currentLocation, customerName: customerName || "Guest", phone: phone || "" };
    setOrders(p => [...p, order]);
    return order;
  };
...
function CustomerView({ placeOrder, completed, isOpen, menu, specials, songOfDay }) {
  ...
  if (screen === "checkout") return (
    <StripeCheckoutScreen
      ...
      onConfirm={(method) => {
        if (method === "stripe") {
          // Stripe handles payment externally; place order and go to waiting
          const order = placeOrder(cart, glitter, profile.name, phone);
  ...
  return (
    <div className="customer-screen">
      ...
      <div className="location-strip"><span>📍 Riverside Park, Buffalo NY</span><span className="mobile-tag">Mobile Stand</span></div>
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── PUNCH CARD ── */
        .music-btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 18px; background: rgba(168,85,247,0.15); border: 1px solid rgba(168,85,247,0.4); border-radius: 999px; color: #d8b4fe; font-size: 12px; font-weight: 600; text-decoration: none; letter-spacing: 1px; transition: all 0.2s; margin-bottom: 12px; }
