...
// ─── DEFAULT MENU ─────────────────────────────────────────────────────────────
const DEFAULT_MENU = [
  { id: 1, name: "Classic Lemonade", price: 3.0, emoji: "🍋", desc: "Fresh squeezed, ice cold, pure perfection.", gradient: "linear-gradient(135deg, #FFE135 0%, #FF9500 100%)", tag: null, sizes: ["Kid Size 12oz","Regular Size 24oz"], sizePrices: [3.0, 5.0], soldOut: false },
  { id: 2, name: "Dragon Fruit Lemonade", price: 5.0, emoji: "🐉", desc: "Dragon fruit syrup with fresh pieces on top. Bold & beautiful.", gradient: "linear-gradient(135deg, #FF6B9D 0%, #C44F8A 100%)", tag: "FAN FAV", sizes: ["Kid Size 12oz","Regular Size 24oz"], sizePrices: [5.0, 7.0], soldOut: false },
  { id: 3, name: "Do You Billieve?", price: 5.0, emoji: "🏈", desc: "Mystery flavor every week. Have faith. Just trust it. #BillsMafia", gradient: "linear-gradient(135deg, #00338D 0%, #4F7FD4 100%)", tag: "MYSTERY", sizes: ["Kid Size 12oz","Regular Size 24oz"], sizePrices: [5.0, 7.0], soldOut: false },
  { id: 4, name: "Prickly Pear Lemonade", price: 5.0, emoji: "🌵", desc: "Sweet, stunning, naturally pink. Gram-worthy every single pour.", gradient: "linear-gradient(135deg, #A855F7 0%, #7C3AED 100%)", tag: "NEW", sizes: ["Kid Size 12oz","Regular Size 24oz"], sizePrices: [5.0, 7.0], soldOut: false },
];
...
function MenuEditor({ menu, setMenu, specials, setSpecials, songOfDay, setSongOfDay }) {
  ...
  const openAdd = () => {
    ...
    setForm({
      name: "", emoji: "🍋", desc: "", tag: "",
      gradient: "linear-gradient(135deg, #FFE135 0%, #FF9500 100%)",
            ...
            <div className="modal-overlay">
              <div className="modal-box">
                ...
                <div style={{ background: "rgba(255,225,53,0.05)", border: "1px solid rgba(255,225,53,0.15)", borderRadius: 12, padding: 12, marginTop: 8, marginBottom: 8 }}>
                  <div style={{ fontSize: 11, color: "#FFE135", fontWeight: 700, letterSpacing: 1, marginBottom: 10 }}>💰 PRICING</div>
                  <div className="modal-field">
    ...
    const attach = () => {
      if (db && firebaseRef && firebaseOnValue) {
        ...
        unsub = firebaseOnValue(profilesRef, (snapshot) => {
          const data = snapshot.val();
          if (data) {
            const found = Object.values(data).sort((a, b) => b.totalSpent - a.totalSpent);
            setProfiles(found);
          } else {
            setProfiles([]);
          }
        ...
        fallbackInterval = setInterval(() => {
          if (db) { clearInterval(fallbackInterval); attach(); }
          else loadProfilesLocal();
        }, 1500);
      }
    };
    attach();
