...
function MusicPlayer({ songOfDay }) {
  ...
  const { ask, loading: aiLoading, response: aiResponse } = useAI();
  ...
  const handleArtistSearch = async () => {
    ...
    setSearching(true);
    setTopSongs([]);
    await ask(`The user searched for this artist or song (possibly misspelled): "${searchQuery}". First correct any spelling errors, then list the top 5 most popular songs by that artist. Reply ONLY with a JSON array like: [{"title":"song name","artist":"correct artist name","spotifyQuery":"song name artist name"},...] No other text.`);
    setSearching(false);
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) setTopSongs(parsed);
    } catch {}
  }, [aiResponse]);
    ...
    <div style={{ marginBottom: 12 }}>
      {!playing && !searchMode ? (
        <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <button className="music-btn" onClick={() => setSearchMode(true)}>🔍 Search Music</button>
        </div>
      ) : searchMode ? (
        <div className="music-search-box">
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            <input className="music-search-input" placeholder="Artist or song (typos ok!)" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && handleArtistSearch()} />
            <button className="music-search-go" onClick={handleArtistSearch} disabled={searching || aiLoading}>{searching || aiLoading ? "..." : "Go"}</button>
          </div>
          {(searching || aiLoading) && <div className="quick-loading"><div className="ai-dots"><span/><span/><span/></div></div>}
          {topSongs.length > 0 && (
            <div className="music-results">
              <div style={{ fontSize: 11, color: "#A855F7", fontWeight: 700, marginBottom: 8, letterSpacing: 1 }}>🎤 {topSongs[0]?.artist?.toUpperCase()}</div>
              {topSongs.map((song, i) => (
                <div key={i} className="music-result-row">
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <a href={`https://open.spotify.com/search/${encodeURIComponent((song.spotifyQuery || song.title + " " + song.artist))}`} target="_blank" rel="noreferrer" className="music-open-btn music-spotify">▶</a>
                    <a href={`https://music.youtube.com/search?q=${encodeURIComponent(song.title + " " + song.artist)}`} target="_blank" rel="noreferrer" className="music-open-btn music-youtube">▶</a>
                  </div>
                </div>
              ))}
            </div>
          )}
          <button className="music-btn" style={{ marginTop: 8, width: "100%" }} onClick={() => { setSearchMode(false); setTopSongs([]); setSearchQuery(""); }}>← Back</button>
        </div>
      ) : (
        <div>
...
// ─── DEFAULT MENU ─────────────────────────────────────────────────────────────
const DEFAULT_MENU = [
  { id: 1, name: "Classic Lemonade (Kids)", price: 3.0, emoji: "🍋", desc: "Fresh squeezed, ice cold, pure perfection. 12oz kids size.", gradient: "linear-gradient(135deg, #FFE135 0%, #FF9500 100%)", tag: null, sizes: ["12oz Kids Plain","12oz Kids Flavored"], sizePrices: [3.0, 5.0], soldOut: false },
  { id: 2, name: "Dragon Fruit Lemonade", price: 7.0, emoji: "🐉", desc: "Dragon fruit syrup with fresh pieces on top. Bold & beautiful.", gradient: "linear-gradient(135deg, #FF6B9D 0%, #C44F8A 100%)", tag: "FAN FAV", sizes: ["18oz Plain","18oz Flavored"], sizePrices: [5.0, 7.0], soldOut: false },
  { id: 3, name: "Do You Billieve?", price: 7.0, emoji: "🏈", desc: "Mystery flavor every week. Have faith. Just trust it. #BillsMafia", gradient: "linear-gradient(135deg, #00338D 0%, #4F7FD4 100%)", tag: "MYSTERY", sizes: ["18oz Plain","18oz Flavored"], sizePrices: [5.0, 7.0], soldOut: false },
  { id: 4, name: "Prickly Pear Lemonade", price: 7.0, emoji: "🌵", desc: "Sweet, stunning, naturally pink. Gram-worthy every single pour.", gradient: "linear-gradient(135deg, #A855F7 0%, #7C3AED 100%)", tag: "NEW", sizes: ["18oz Plain","18oz Flavored"], sizePrices: [5.0, 7.0], soldOut: false },
];
...
function DailyGoalTracker({ totalToday }) {
  ...
  const color = pct >= 100 ? "#4ade80" : pct >= 60 ? "#FFE135" : "#FF9500";
  ...
  return (
    <div className="goal-tracker">
      ...
      <div className="goal-stats">
        ...
        <span style={{ marginLeft: "auto", color: pct >= 100 ? "#4ade80" : "#888" }}>
    ...
    <div style={{ padding: "0 16px" }}>
      <div className="banking-card">
        ...
        {!permGranted ? (
          ...
          <div>
            <p style={{ fontSize: 13, color: "#4ade80", marginBottom: 12 }}>✅ Notifications enabled!</p>
            <button className="stripe-btn" onClick={sendLive} style={{ background: "linear-gradient(135deg, #22c55e, #16a34a)" }}>
    ...
    <div className="order-banner" onClick={onTap}>
      <div className="order-banner-left">
        ...
        <div className="order-banner-text">
          <div style={{ fontSize: 12, fontWeight: 700, color: "#FFE135" }}>Your order is being made!</div>
    ...
    <div className="pin-overlay">
      <div className="pin-box">
        ...
        <div style={{ background: "#0f0f1a", border: "1px solid #2a2a3e", borderRadius: 14, padding: 16, margin: "16px 0", width: "100%" }}>
          <div style={{ fontSize: 11, color: "#FFE135", fontWeight: 700, marginBottom: 8 }}>YOUR BACKUP CODE</div>
...
function CustomerView({ placeOrder, completed, isOpen, menu, specials, songOfDay, currentLocation }) {
  ...
  const [activeOrder, setActiveOrder] = useState(() => {
    try { const s = localStorage.getItem("rs_active_order"); return s ? JSON.parse(s) : null; } catch { return null; }
  });
  ...
  useEffect(() => {
    if (activeOrder) try { localStorage.setItem("rs_active_order", JSON.stringify(activeOrder)); } catch {}
    else try { localStorage.removeItem("rs_active_order"); } catch {}
  }, [activeOrder]);
  ...
  useEffect(() => {
    ...
    if (found && !prevCompleted.current.find(o => o.id === found.id)) {
      ...
      const updated = { ...profile, punches: newPunches, totalOrders: profile.totalOrders + 1, totalSpent: profile.totalSpent + activeOrder.total, favoriteItems: newFavorites, orderHistory: [...profile.orderHistory, { items: activeOrder.items.map(i => ({ id: i.id, name: i.name, emoji: i.emoji, price: i.price })), total: activeOrder.total, date: new Date().toLocaleDateString(), location: activeOrder.location?.name || "Park" }].slice(-20) };
  ...
  if (showWaitingFullscreen && activeOrder) return (
    <div className="full-screen waiting-screen">
      ...
      <div className="waiting-content">
        ...
        {bleacherSection && <p style={{ color: "#A855F7", fontSize: 13, marginTop: 8 }}>📍 {bleacherSection}</p>}
  ...
  if (screen === "checkout") return (
    <StripeCheckoutScreen total={total} cart={cart} glitter={glitterTier} glitterPrice={glitterPrice}
      ...
      onConfirm={async (method) => {
        ...
        setActiveOrder(order); setScreen("menu"); setShowBanner(true); setCart([]); setGlitterTier(null); setOrderNote("");
      }}
    />
  );
        ...
        {cart.length === 0 ? <p className="empty-msg">Nothing added yet!</p> : (
          <>
            ...
            <div style={{ padding: "12px 0", borderBottom: "1px solid #1a1a2e" }}>
              <div style={{ fontSize: 11, color: "#A855F7", fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>✨ ADD EDIBLE GLITTER</div>
            ...
            {availableSections.length > 0 && (
              <div style={{ padding: "12px 0", borderBottom: "1px solid #1a1a2e" }}>
                <div style={{ fontSize: 11, color: "#60a5fa", fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>📍 WHERE ARE YOU SITTING?</div>
  ...
  return (
    <div className="customer-screen">
      ...
      <div className="menu-list">
        {menu.map((item, idx) => (
          <div key={item.id} className={`menu-card ${item.soldOut ? "sold-out" : ""}`} style={{"--grad":item.gradient, animationDelay:`${idx*0.08}s`}}>
            ...
            <div className="card-body">
              ...
              <div className="card-bottom">
                ...
                <button className="add-btn" onClick={() => addToCart(item)} disabled={item.soldOut}>{item.soldOut ? "Sold Out" : "+ Add"}</button>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className="owner-screen">
          ...
          : orders.map(order => (
            <div key={order.id} className="order-card">
              ...
              <div className="order-info">
                ...
                {order.bleacherSection && <div className="order-line" style={{color:"#60a5fa"}}>📍 {order.bleacherSection}</div>}
                {order.note && <div className="order-line" style={{color:"#FFE135"}}>📝 {order.note}</div>}
      ...
      {tab === "menu" && (
        <div style={{padding:"12px 16px"}}>
          <div style={{fontSize:13,color:"#888",marginBottom:12}}>Toggle sold out to hide items from customers</div>
          {menu.map(item => (
            <div key={item.id} className="editor-row">
              <div className="editor-row-info">
                ...
                <div>
                  ...
                  <div className="editor-row-price">${item.price.toFixed(2)}</div>
                </div>
              </div>
              <button
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .glitter-tier-btn.active { background: rgba(168,85,247,0.15); border-color: #A855F7; color: #d8b4fe; }
        .glitter-tier-desc { font-size: 11px; color: #666; flex: 1; }
        .glitter-tier-price { font-size: 12px; font-weight: 700; color: #FFE135; margin-left: auto; }
        ...
        .bleacher-btn.active { background: rgba(96,165,250,0.15); border-color: #60a5fa; color: #60a5fa; font-weight: 600; }
        ...
        .order-banner-num { font-family: 'Bebas Neue', sans-serif; font-size: 32px; color: #FFE135; line-height: 1; }
        .order-banner-text { flex: 1; }
        .order-banner-ping { width: 10px; height: 10px; border-radius: 50%; background: #FFE135; animation: blink 1.2s infinite; }

        /* ── SOLD OUT ── */
        .menu-card.sold-out { filter: grayscale(0.6); }
        .sold-out-overlay { position: absolute; inset: 0; background: rgba(0,0,0,0.55); display: flex; align-items: center; justify-content: center; font-family: 'Bebas Neue', sans-serif; font-size: 24px; letter-spacing: 3px; color: #f87171; border-radius: 20px; z-index: 5; }
        ...
        .goal-title { font-size: 12px; font-weight: 700; color: #4ade80; letter-spacing: 1px; }
        .goal-edit-btn { background: none; border: none; color: #4ade80; font-size: 12px; cursor: pointer; font-family: 'Outfit', sans-serif; }
        ...
        .cash-log-title { font-family: 'Bebas Neue', sans-serif; font-size: 18px; color: #FFE135; letter-spacing: 1px; }
        .cash-log-total { font-family: 'Bebas Neue', sans-serif; font-size: 22px; color: #4ade80; }
        ...
        .cash-log-amount { font-size: 14px; font-weight: 700; color: #4ade80; }

        /* ── SOLD OUT TOGGLE ── */
        .soldout-toggle { padding: 8px 12px; border-radius: 10px; border: none; font-size: 12px; font-weight: 600; cursor: pointer; font-family: 'Outfit', sans-serif; background: rgba(74,222,128,0.1); color: #4ade80; border: 1px solid rgba(74,222,128,0.3); }
        .soldout-toggle.sold { background: rgba(248,113,113,0.1); color: #f87171; border-color: rgba(248,113,113,0.3); }
        ...
        .music-search-input:focus { border-color: #A855F7; }
        .music-search-go { padding: 10px 16px; background: linear-gradient(135deg, #A855F7, #7C3AED); border: none; border-radius: 10px; color: white; font-weight: 700; font-size: 14px; cursor: pointer; font-family: 'Outfit', sans-serif; }
      ...
      <div className="app-root">
        ...
        <nav className="bottom-nav">
          ...
          <button
            ...
            style={{ position: "relative", opacity: ownerUnlocked ? 1 : 0.01, minWidth: 60 }}
            aria-hidden={!ownerUnlocked}
          >
            <span>{ownerUnlocked ? "📊" : "·"}</span>
            <span>{ownerUnlocked ? "Owner" : ""}</span>
