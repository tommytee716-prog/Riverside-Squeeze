...
// Save active order to Firebase (so it follows the customer across devices) + local cache for speed
function saveActiveOrder(phone, order) {
  const digits = phone.replace(/\D/g, "");
  try {
    if (order) localStorage.setItem(getActiveOrderKey(phone), JSON.stringify(order));
    else localStorage.removeItem(getActiveOrderKey(phone));
  } catch {}
  if (db && firebaseSet && firebaseRef && digits) {
    if (order) firebaseSet(firebaseRef(db, `activeOrders/${digits}`), order);
    else firebaseSet(firebaseRef(db, `activeOrders/${digits}`), null);
  }
}
...
function CustomerView({ placeOrder, completed, isOpen, menu, specials, songOfDay, currentLocation }) {
  ...
  const lastFirebaseOrderJSON = useRef(undefined); // tracks the exact value last received FROM Firebase, so we know not to echo it back

  // Once a phone number is known, live-listen to that phone's active order in Firebase (cross-device truth)
  useEffect(() => {
    if (!phone) return;
    const digits = phone.replace(/\D/g, "");
    let unsub = null;
    let pollId = null;
    let cancelled = false;
    lastFirebaseOrderJSON.current = undefined; // reset tracking on every fresh phone login

    const applyOrder = (restored) => {
      // Safety check: never show an order that's already marked completed
      const finalVal = (restored && completed.find(o => o.id === restored.id)) ? null : (restored || null);
      lastFirebaseOrderJSON.current = JSON.stringify(finalVal);
      setActiveOrder(finalVal);
    };

    const attachLiveListener = () => {
      if (cancelled) return;
      const orderRef = firebaseRef(db, `activeOrders/${digits}`);
      unsub = firebaseOnValue(orderRef, (snapshot) => {
        const val = snapshot.val();
        try { if (val) localStorage.setItem(getActiveOrderKey(phone), JSON.stringify(val)); else localStorage.removeItem(getActiveOrderKey(phone)); } catch {}
        applyOrder(val);
      });
    };
    ...
    const waitForFirebase = () => {
      ...
      if (db && firebaseRef && firebaseOnValue) {
        ...
        // Firebase still booting — show local cache meanwhile, keep checking
        try {
          const s = localStorage.getItem(getActiveOrderKey(phone));
          applyOrder(s ? JSON.parse(s) : null);
        } catch { applyOrder(null); }
        pollId = setTimeout(waitForFirebase, 500);
      }
    };
    waitForFirebase();
  ...
  // Whenever activeOrder changes locally (placing a new order, clearing it), push to Firebase + cache —
  // but only if this change didn't just come FROM Firebase (compare against last known Firebase value)
  useEffect(() => {
    if (!phone) return;
    const currentJSON = JSON.stringify(activeOrder);
    if (currentJSON === lastFirebaseOrderJSON.current) return; // this is an echo of what Firebase just told us, skip
    lastFirebaseOrderJSON.current = currentJSON; // this IS the new source of truth now
    saveActiveOrder(phone, activeOrder);
  }, [activeOrder, phone]);
  ...
  useEffect(() => {
    ...
    if (found && !processedOrderIds.current.has(found.id)) {
      processedOrderIds.current.add(found.id);
      ...
      setProfile(currentProfile => {
        const base = currentProfile || profile;
        const newPunches = (base.punches + orderItems.length) % 6;
        const newFavorites = { ...base.favoriteItems };
        orderItems.forEach(item => { newFavorites[String(item.id)] = (newFavorites[String(item.id)] || 0) + 1; });
        const updated = { ...base, punches: newPunches, totalOrders: base.totalOrders + 1, totalSpent: base.totalSpent + (activeOrder.total || 0), favoriteItems: newFavorites, orderHistory: [...(base.orderHistory || []), { items: orderItems.map(i => ({ id: i.id, name: i.name, emoji: i.emoji, price: i.price })), total: activeOrder.total || 0, date: new Date().toLocaleDateString(), location: activeOrder.location?.name || "Park" }].slice(-20) };
  ...
  if (screen === "login") return <PhoneLoginScreen onLogin={(p, ph) => {
    ...
    setShowBanner(true);
    processedOrderIds.current = new Set();
    setProfile(p);
    setPhone(ph);
    setScreen("menu");
  }} />;
...
// ─── OWNER VIEW ───────────────────────────────────────────────────────────────
function OwnerView({ orders, completeOrder, totalToday, totalOrders, locationStats, sales, isOpen, setIsOpen, currentLocation, setCurrentLocation, menu, setMenu, specials, setSpecials, songOfDay, setSongOfDay, saveStatus, onLock }) {
  ...
  return (
    <div className="owner-screen">
      ...
      {saveStatus !== "idle" && (
        <div className={`save-status-banner save-status-${saveStatus}`}>
          {saveStatus === "saving" && "💾 Saving changes..."}
          {saveStatus === "saved" && "✅ Saved — live on all devices now"}
          {saveStatus === "error" && "⚠️ Couldn't sync — check your internet connection"}
        </div>
      )}
...
function AppInner() {
  ...
  const [specials, setSpecialsState] = useState(DEFAULT_SPECIALS);
  const [songOfDay, setSongOfDayState] = useState({ title: "Started From the Bottom", artist: "Drake", vibe: "Buffalo vibes only 🏈" });
  ...
  useEffect(() => {
    ...
    const listenForStatus = async () => {
      ...
      // Listen for menu changes — so editing on one device updates every device live
      const menuRef = firebaseRef(db, "menu");
      unsubMenu = firebaseOnValue(menuRef, (snapshot) => {
        const val = snapshot.val();
        if (val) setMenuState(Array.isArray(val) ? val : Object.values(val));
      });
      // Listen for specials changes
      const specialsRef = firebaseRef(db, "specials");
      unsubSpecials = firebaseOnValue(specialsRef, (snapshot) => {
        const val = snapshot.val();
        if (val) setSpecialsState(Array.isArray(val) ? val : Object.values(val));
      });
      // Listen for song of the day changes
      const songRef = firebaseRef(db, "songOfDay");
      unsubSong = firebaseOnValue(songRef, (snapshot) => {
        const val = snapshot.val();
        if (val) setSongOfDayState(val);
      });
    };
    listenForStatus();
    return () => { if (unsub) unsub(); if (unsubLoc) unsubLoc(); if (unsubMenu) unsubMenu(); if (unsubSpecials) unsubSpecials(); if (unsubSong) unsubSong(); };
  }, []);
  ...
  // Generic save-to-Firebase helper with visible status feedback for the owner
  const saveToFirebase = (path, value) => {
    setSaveStatus("saving");
    if (db && firebaseSet && firebaseRef) {
      firebaseSet(firebaseRef(db, path), value)
        .then(() => { setSaveStatus("saved"); setTimeout(() => setSaveStatus("idle"), 2000); })
        .catch(() => { setSaveStatus("error"); setTimeout(() => setSaveStatus("idle"), 3000); });
    } else {
      // Firebase not ready — still update locally but flag that it didn't sync
      setSaveStatus("error");
      setTimeout(() => setSaveStatus("idle"), 3000);
    }
  };

  // setMenu/setSpecials/setSongOfDay accept either a new value or an updater function (like normal useState setters),
  // apply it locally immediately, AND push the resulting value to Firebase so every device picks it up live.
  const setMenu = (updater) => {
    setMenuState(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      saveToFirebase("menu", next);
      return next;
    });
  };
  const setSpecials = (updater) => {
    setSpecialsState(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      saveToFirebase("specials", next);
      return next;
    });
  };
  const setSongOfDay = (updater) => {
    setSongOfDayState(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      saveToFirebase("songOfDay", next);
      return next;
    });
  };
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .save-status-banner { text-align: center; padding: 8px 16px; font-size: 12px; font-weight: 700; animation: slideIn 0.25s ease both; }
        .save-status-saving { background: rgba(255,238,0,0.1); color: #FFEE00; }
        .save-status-saved { background: rgba(26,232,92,0.12); color: #3DFF8F; }
        .save-status-error { background: rgba(255,45,45,0.12); color: #FF5252; }
